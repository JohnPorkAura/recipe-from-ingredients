...
<title>Recipe from Ingredients</title>
...
:root{
  --acc:#84cc16;--acc2:#a3e635;--bg:#050900;
...
<div id="auth-overlay">
  <div class="auth-panel-left">
    <div class="auth-panel-brand">Recipe from Ingredients</div>
    <div class="auth-panel-sub">List what's in your fridge. Get a delicious recipe with full instructions. Built for users.</div>
  </div>
  <div class="auth-panel-right">
    <div class="auth-box">
      <div class="auth-logo">Recipe from Ingredients</div>
      <div class="auth-sub-txt">Cook something great with what you have</div>
      <div class="auth-tabs">
...
<header id="main-header" style="display:none">
  <div style="display:flex;align-items:center;gap:12px">
    <div class="logo"><span>Recipe from Ingredients</span></div>
...
<div id="main-app" style="display:none">
  <div class="hero-band">
    ...
    <h1><em>Recipe from Ingredients</em><br>Cook something great with what you have</h1>
    <p>List what's in your fridge. Get a delicious recipe with full instructions. Built for users.</p>
  </div>
  ...
  <div class="below-hero">
    <div class="card" id="input-card">
      <div class="card-hd">
        ...
        <button class="card-action" onclick="clearForm()">Clear</button>
          ...
          <div class="field">
      <label for="field-ingredients">Ingredients</label>
      <textarea id="field-ingredients" placeholder="Enter ingredients..." required></textarea>
    </div>
    <div class="field">
      <label for="field-cuisine_style">Cuisine Style</label>
      <select id="field-cuisine_style">
        <option value="">— Select —</option>
      <option value="Whatever works best">Whatever works best</option>
    ...
    <div class="field">
      <label for="field-dietary_needs">Dietary Needs</label>
      <select id="field-dietary_needs">
        <option value="">— Select —</option>
    ...
    <div class="field">
      <label for="field-time_available">Time Available</label>
      <select id="field-time_available">
        <option value="">— Select —</option>
    ...
    <footer>
      <div class="foot-note">Recipe from Ingredients · Powered by Groq llama-3.3-70b</div>
...
<script>
const APP={name:'Recipe from Ingredients',mode:'fastapi',auth:false,systemPrompt:"You are a specialised AI tool: Recipe from Ingredients.\\n\\nYou are a knowledgeable advisor. Give specific, personalised advice based on what the user has shared.\\n\\nYour job: List what's in your fridge. Get a delicious recipe with full instructions.\\n\\nThe user will provide: ingredients, cuisine style, dietary needs, time available. Use all of this information to personalise your response.\\n\\nOutput format: Structure your response as clear numbered steps. Each step on its own line, actionable and specific. Tell the user exactly what to do.\\n\\nQuality rules:\\n- Be specific to what the user has actually provided \\u2014 never give generic advice that ignores their inputs\\n- Do not start with filler openers like Sure, Great, Certainly, or Of course\\n- Do not explain what you are about to do \\u2014 just do it\\n- Do not add unnecessary disclaimers unless they are genuinely important\\n- If the user's input is vague, make reasonable assumptions and state them briefly\\n- Aim for depth over length \\u2014 one specific, useful insight beats three generic ones",promptTemplate:"Ingredients: {{ingredients}}\\nCuisine Style: {{cuisine_style}}\\nDietary Needs: {{dietary_needs}}\\nTime Available: {{time_available}}",resultFormat:'steps',resultConfig:{"label": "Result", "copyable": true},inputs:[{"id": "ingredients", "label": "Ingredients", "type": "textarea", "placeholder": "Enter ingredients...", "required": true}, {"id": "cuisine_style", "label": "Cuisine Style", "type": "select", "options": ["Whatever works best", "Italian", "Asian", "Mexican", "Mediterranean", "American", "Indian"], "required": false}, {"id": "dietary_needs", "label": "Dietary Needs", "type": "select", "options": ["None", "Vegetarian", "Vegan", "Gluten-free", "Dairy-free", "Low carb"], "required": false}, {"id": "time_available", "label": "Time Available", "type": "select", "options": ["Whatever it takes", "Under 20 minutes", "Under 30 minutes", "Under 1 hour"], "required": false}]};
...
@app.get("/api/health")
def health():
    return {"status": "ok", "app": "Recipe from Ingredients", "ai_ready": bool(GROQ_API_KEY)}
